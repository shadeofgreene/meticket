	...
	if (request.user.userId && request.collection) {
		...
		if (request.collection === 'Office') {
			...
			tickets.find(function (err, lTickets) {
				// get tickets that have not been saved to the server yet
				...
				if (ticketsWithoutId.length > 0) {
					...
					var saveLocalTicketsToServerOptions = {
						host: hostConstant,
						path: '/TicketSystem/TicketView/SaveTickets',
						method: 'POST',
						headers: {
							'Content-Type': 'application/json;charset=UTF-8',
					...
					var chunks = '';
					http.request(saveLocalTicketsToServerOptions, function (res) {
						res.on('data', function (chunk) {
							chunks += chunk;
						});
					}).write(JSON.stringify(ticketsWithoutId)).end(function() {
